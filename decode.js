...
const encode = (input) =>
	[...input]
		.map((x, i) => {
			console.log([x.charCodeAt(0), i])
			return [x.charCodeAt(0), i]
		})
		.sort()
		.flatMap((x) => x)
		.join('.')
		.match(/./g)
		.flatMap((x, i) => {
			return new Array(x == '.' ? 1 : 2 + x * 2).fill((1 + i) % 2)
		})
		.join('')
		.replace(/(([01])\2*)/g, (x) => {
			return `${+x ? '.' : '-'}${x.length}`
			...
			(x) =>
				`${
					x.includes('.')
						? [x.slice(1)]
								.flatMap((x) =>
									new Array(Number(x)).fill(1, 0, Number(x))
								)
								.join('')
						: [x.slice(1)]
								.flatMap((x) =>
									new Array(Number(x)).fill(0, 0, Number(x))
								)
								.join('')
				}`
		)
		.replace(/101|010|01|10/g, (x) =>
			x.length === 3
				? `${x.slice(0, 1)},${x.slice(1, 2)},${x.slice(2)}`
				: `${x.slice(0, 1)},${x.slice(1)}`
		)
		.split(',')
		.map((x) => (x.length === 1 ? '.' : ((x.length - 2) / 2).toFixed(0)))
		.join('')
		.split('.')
		.map((item, index, array) =>
			index % 2 === 0
				? [parseInt(item), parseInt(array[index + 1])]
				: null
		)
...
let firstDecode = decode(inputData)
let secondDecode = decode(firstDecode)

console.log('decode ', secondDecode)

// Ответ: i love puzzles <3
